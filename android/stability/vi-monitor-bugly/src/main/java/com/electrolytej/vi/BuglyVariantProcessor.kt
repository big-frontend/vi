...
@AutoService(BaseSymbolVariantProcessor::class)
class BuglyVariantProcessor : BaseSymbolVariantProcessor() {
    companion object {
...
//        if (!project.plugins.hasPlugin("com.android.application")) throw  IllegalArgumentException("bugly plugin必须在app模块中配置")
    ...
    private fun postSymbol(r: SymbolReq) {
        ...
        println(r)
        println(r.file.name)
        val form = MultipartBody.Builder().setType(MultipartBody.FORM).addFormDataPart("api_version", "1")
                ...
                .addFormDataPart(
                    "file", r.file.name, r.file.asRequestBody("text/plain".toMediaTypeOrNull())
                ).build()
