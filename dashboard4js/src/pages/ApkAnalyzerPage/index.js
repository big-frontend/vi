      ...
      <Spin size="large" spinning={loading}>
        ...
        <Collapse defaultActiveKey={['1']} onChange={onChange}>
          {task3?.show_file_sizes?.length > 0 &&
            <Panel
              header={`${task3?.task_description}\t\t数量:${task3?.show_file_sizes?.length}  超过10k的图片可以放到图床`}
              key={task3?.task_type}>
              {task3?.show_file_sizes?.map((e) => <p>{e.entry_name} {(e.entry_size / 1024).toFixed(2)}k</p>)}
            </Panel>
          }
          {task14?.unstripped_lib?.length > 0 &&
            <Panel header={`${task14?.task_description}\t\t数量:${task14?.unstripped_lib?.length}`}
                   key={task14?.task_type}>
          ...
          {task12?.unused_resources?.length > 0 &&
            <Panel header={`${task12?.task_description}\t\t数量:${task12?.unused_resources?.length}`}
                   key={task12?.task_type}>
              {task12?.unused_resources?.map((e) => <p>{e.name}</p>)}
            </Panel>
          }
          {task13?.unused_assets?.length > 0 &&
            <Panel
              header={`${task13?.task_description}\t\t数量:${task13?.unused_assets?.length} 最大可减:${max_reduce_size13.toFixed(2)}k`}
              key={task13?.task_type}>
              {task13?.unused_assets?.map((e) => <p>{e.name}</p>)}
            </Panel>
          }
          {task10?.duplicated_files?.length > 0 &&
            <Panel
              header={`${task10?.task_description}\t\t数量:${task10?.duplicated_files?.length}  最大可减:${max_reduce_size10.toFixed(2)}k`}
              key={task10?.task_type}>
              {task10?.duplicated_files?.map((e) =>
                <p>大小：{(e.size / 1024).toFixed(2)}k {e.duplicated_file_paths.map((a) => <p>{a.name}</p>)}</p>)}
            </Panel>
          }
          {task8?.uncompressed_file_types?.length > 0 &&
            <Panel
              header={`${task8?.task_description}\t\t数量:${task8?.uncompressed_file_types?.length} 最大可减:${max_reduce_size8.toFixed(2)}k 某个文件类型的所有文件都没有经过压缩，可以考虑是否需要压缩`}
              key={task8?.task_type}>
          ...
          {task6?.non_alpha_pngs?.length > 0 &&
            <Panel
              header={`${task6?.task_description}\t\t数量:${task6?.non_alpha_pngs?.length} 最大可减:${max_reduce_size6.toFixed(2)}k 对于不含alpha通道的png文件，可以转成jpg格式来减少文件的大小`}
              key={task6?.task_type}>
